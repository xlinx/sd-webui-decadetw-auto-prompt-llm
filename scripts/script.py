        ...
        with gr.Blocks():
            # gr.Markdown("Blocks")
                ...
                with gr.Accordion(open=True, label="Auto LLM Prompt v20240723"):
                    ...
                    with gr.Tab("Prompt/LLM-JS"):
                        # with gr.Accordion(open=True, label="[Prompt]/[LLM-JS]"):
                        gr.Markdown("* Generate by JS / each generate \n"
                                    "* inactive between SD-Prompt and LLM-Prompt")
                        ...
                        with gr.Row():
                            js_prompt_js = gr.Textbox(label=" f[Prompt-JS]  %%{...}%%", lines=33,
                                                      placeholder=self.defaultJS)
                            js_result = gr.Textbox(label=" [Prompt-JS-Result]", lines=33, value="")
                    with gr.Tab("Setup"):
                        llm_apiurl = gr.Textbox(label="0. [LLM-URL]", lines=1,
                                                placeholder="http://localhost:1234/v1")
                        llm_apikey = gr.Textbox(label="0. [LLM-API-Key]", lines=1,
                                                placeholder="lm-studio")
        return [js_is_enabled, llm_is_enabled, llm_apiurl, llm_apikey,
